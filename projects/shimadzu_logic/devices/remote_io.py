...
class AutonicsEIPClient:
    ...
    def _parse_to_bit_list(self, status_value, bit_count=16):
        """
        10진수 상태 값을 16비트 배열 ([1, 0, 0, ...])로 변환합니다.
        """
    ...
    def _read_data_and_print(self, instance_id, instance_name):
        ...
        try:
            ...
            if response.error:
                ...
                # 데이터 해석 (16비트 = 2바이트 데이터)
                if isinstance(raw_data, bytes):
                    print(f"   -> 수신된 데이터 크기: {len(raw_data)} 바이트")
                    if len(raw_data) >= 2:
                        # little-endian으로 2바이트를 부호 없는 정수로 변환
                        status_int = struct.unpack('<H', raw_data[:2])[0] 
                        bit_list = self._parse_to_bit_list(status_int, bit_count=16)
                        ...
                        print(f"   -> 오류: 수신된 데이터 길이가 2바이트보다 짧습니다. 인스턴스 ID({instance_id}) 또는 데이터 길이 확인 필요.")
                        return None
    ...
    def write_output_data(self, input_bits: list, writing_class=CIP_CLASS_ASSEMBLY, writing_instance=OUTPUT_ASSEMBLY_INSTANCE, writing_attribute=CIP_ATTRIBUTE_DATA, writing_service=SERVICE_WRITE_DATA):
        """
        [DO(디지털 출력) 쓰기 전용 함수]
        16개의 비트 상태를 담은 리스트를 입력받아 DO 값을 쓰고, 즉시 다시 읽어 쓰기 성공 여부를 확인합니다.
        
        매개변수:
        input_bits (list): 16개의 정수 (0 또는 1)로 구성된 리스트. [DO0, DO1, ..., DO15] 순서.
        """
        if len(input_bits) != 16 or any(bit not in [0, 1] for bit in input_bits):
            print("❌ 오류: input_bits는 길이가 16인 0 또는 1 값의 리스트여야 합니다.")
            return
        ...
        # 값을 바이트 형식으로 변환 (DO16N은 16비트 = 2바이트 WORD 가정)
        try:
            value_bytes = struct.pack('<H', value_to_write)
        except Exception as e:
...
if __name__ == '__main__':
    ...
    if APIO_C_EI_IP:
        try:
            client = AutonicsEIPClient(APIO_C_EI_IP)
