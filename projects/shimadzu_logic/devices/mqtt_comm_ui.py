...
class MqttCommUi:
    ...
    def _on_connect(self, client, userdata, flags, rc):
        """브로커 연결 시 호출되는 콜백 함수"""
        if rc == 0:
            if DEBUG_MODE: print(f"→ [INFO] MQTT 연결 성공. 토픽 구독 시작: {TOPIC_LOGIC_EVT}")
            # Logic에서 오는 ACK/이벤트 수신 토픽 구독
            client.subscribe(TOPIC_LOGIC_EVT) 
        else:
            if DEBUG_MODE: print(f"→ [ERROR] MQTT 연결 실패: 반환 코드 {rc}")
    ...
    def _on_message(self, client, userdata, msg):
        ...
        try:
            ...
            if DEBUG_MODE: print(f"→ [ERROR] JSON 디코딩 오류: {msg.payload.decode('utf-8')}")
        except Exception as e:
            if DEBUG_MODE: print(f"→ [ERROR] 메시지 처리 중 오류 발생: {e}")
    ...
    def connect_and_loop(self):
        ...
        try:
            ...
            if DEBUG_MODE: print("UI 클라이언트 루프 시작.")
        except ConnectionRefusedError:
            if DEBUG_MODE: print(f"!!! [CRITICAL] {self.host}:{self.port}에 MQTT 브로커가 실행 중인지 확인하십시오. !!!")
        except Exception as e:
            if DEBUG_MODE: print(f"MQTT 연결 중 알 수 없는 오류 발생: {e}")

    def disconnect(self):
        """MQTT 연결을 종료합니다."""
        self.client.loop_stop()
        self.client.disconnect()
        if DEBUG_MODE: print("UI 클라이언트 연결 종료.")
...
if __name__ == "__main__":
    ...
    if DEBUG_MODE: print(f"전송된 START 명령 ID: {command_id}")
    time.sleep(2)
    ...
    if DEBUG_MODE: print(f"전송된 DO 명령 ID: {command_id}")
    time.sleep(2)
    
    # 3. Bin Picking 명령 시뮬레이션
    command_id = ui_comm.send_binpick_command("go_home", "JOB-BP-001")
    if DEBUG_MODE: print(f"전송된 BINPICK 명령 ID: {command_id}")
    time.sleep(5)

    ui_comm.disconnect()
    if DEBUG_MODE: print("UI 통신 테스트 종료.")