...
def Rot_xyz(xr, yr, zr):
    '''
    xyz rotatio matrix - caution: axis order: x,y,z,
    roll pitch yaw rotation, reverse order
    '''
    return np.matmul(np.matmul(Rot_axis(1,xr),Rot_axis(2,yr)),Rot_axis(3,zr))
...
def diff_cmd(cmd1, cmd2):
    """
    cmd1에서 cmd2로 이동하기 위한 상대 변위를 반환한다.
    반환값 형식은 [dx, dy, dz, d_roll°, d_pitch°, d_yaw°].
    """
    T1 = cmd2T(cmd1)
    T2 = cmd2T(cmd2)
    delta = SE3_inv(T1) @ T2
